"""Testing PAA implementation."""

import numpy as np
import pytest

from saxpy import distance


def test_euclidean():
    """Test euclidean distance."""
    assert pytest.approx(np.sqrt(2), 0.0000001) == distance.euclidean(
        np.array([1.0, 1.0]), np.array([2.0, 2.0])
    )

    a = np.array([0.5, 0.8, 0.9])
    b = np.array([-0.15, 0.38, 0.92])
    assert pytest.approx(0.7741447, 0.0000001) == distance.euclidean(a, b)


def test_early_abandoned():
    """Test euclidean distance."""
    assert pytest.approx(np.sqrt(2), 0.0000001) == distance.early_abandoned_euclidean(
        np.array([1.0, 1.0]), np.array([2.0, 2.0]), np.inf
    )

    a = np.array([0.5, 0.8, 0.9])
    b = np.array([-0.15, 0.38, 0.92])
    assert pytest.approx(0.7741447, 0.0000001) == distance.early_abandoned_euclidean(a, b, np.inf)

    assert 1 == np.isnan(distance.early_abandoned_euclidean(a, b, 0.1))
